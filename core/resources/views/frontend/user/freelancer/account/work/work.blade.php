        ...
        <ul class="setup-wrapper-work-list get_subcategory overflow-auto set_scroll_height">
			...
			$sub_categories  = \Modules\Service\Entities\SubCategory::where('category_id',$user_work->category_id)->get();?>
                @foreach($sub_categories as $sub_cat)
                    <li class="setup-wrapper-work-list-item get_subcategory choose_a_subcategory @if(in_array($sub_cat->id,$subcategory_ids) && $user_work->user_id === $user_id) active @endif" data-id="{{ $sub_cat->id }}">{{ $sub_cat->sub_category }}</li>
                @endforeach
            @else
